#ifndef __Hologram_BITMAP_h
#define __Hologram_BITMAP_h
...
int creatBitmapFile(unsigned char* pixelbuffer, int pic_width, int pic_height, uint16_t bitsperpixel,  const char* file_name) {
	...
	if (pos == std::string::npos)
		sprintf(bmpFile, "%s.bmp", fname.c_str());
	else
	{
		if (strcmp(fname.substr(pos).c_str(), ".bmp") == 0)
			sprintf(bmpFile, "%s", fname.c_str());
		else
			sprintf(bmpFile, "%s.bmp", fname.c_str());
	}
    ...
    fwrite(img_tmp,1,_pixelbytesize,fp);
    fclose(fp);
    free(pbitmap);
	free(img_tmp);
	return 1;
...
{
	char bmpFile[256];
	sprintf(bmpFile, "%s", file_name);
	FILE *infile = fopen(bmpFile, "rb");
