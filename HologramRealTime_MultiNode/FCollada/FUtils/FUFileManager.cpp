...
fstring FUFileManager::GetFile(const fstring& filename)
{
	fchar fullPath[MAX_PATH + 1];
	fstrncpy(fullPath, filename.c_str(), MAX_PATH);
	fullPath[MAX_PATH] = 0;
	...
	lastSlash = max(lastSlash, lastBackslash);
	//if (lastSlash != NULL) *(lastSlash + 1) = 0;
	return fstring(lastSlash + 1);
}

fstring FUFileManager::GetFileWithoutExtension(const fstring& _filename)
{
	fstring file_only = GetFile(_filename);

	fchar filename[MAX_PATH];
	fstrncpy(filename, file_only.c_str(), MAX_PATH);

	filename[MAX_PATH - 1] = 0;

	fchar* lastPeriod = fstrrchr(filename, '.');
	if (lastPeriod == NULL) return fstring(filename);
